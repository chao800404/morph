...
- **元素目標解析同樣只能有一份**：`@/lib/storefront/ast/element-target`。AST patch、
  Inspector 的鎖定判斷與即時樣式預覽都必須經過它。這三處曾各自實作，結果是「選得到
  但控制項被禁用」「能寫回原始碼但拖曳沒有回饋」這類無訊息的半殘狀態。
- 樣式套用會重繪 Theme，選取指向的 DOM 節點會被替換。預覽必須在節點離開文件時
  重新附著選取，否則覆蓋層會追蹤一個已卸離的節點，選取框無聲消失。
