...
- **從媒體庫複製到 `public/` 是快照，不是連結**：`copyAssetToThemePublic` 只收資產 id 與目標路徑，由伺服器從媒體庫的 R2 讀 bytes（先以資料列大小、再以實際 bytes 判上限），再走同一個 `saveBinaryFile`（`expectMissing`，格式依 bytes 判定，SVG／影片一律拒絕）。bytes 不經瀏覽器，client 無法冒充來源。之後媒體庫改動或刪除不影響這份複本。介面（Code 模式 Assets 面板、Assets 的 Site public/）必須在複製前說明：屬於網站、發布後公開、不能設為私有、與媒體庫脫鉤。Code 模式仍不提供把媒體庫 URL 寫進原始碼的方式。
...
- **SVG 先隔離送出，才可能接受上傳**：平台送出的每個 `image/svg+xml` 回應都帶 `theme-svg-isolation.ts` 的 `SVG_ISOLATION_HEADERS`（`sandbox` CSP、`default-src 'none'`、`nosniff`），`<img>` 照常顯示，直接開啟或當文件嵌入時不得執行腳本。四條送出路徑都要有：Live Preview 的 Vite 中介層（產生的 `vite.config.ts`，Sandbox 與本地 sidecar 共用）與 Sandbox 預覽代理、`serveThemeArtifact`（建置預覽與正式店面的 client 檔案）、正式店面 Theme Worker 的回應、Theme Worker 靜態資產（部署器寫入的平台 `_headers`；建置產物自己的 `_headers`／`_redirects` 一律不部署）。伺服器的文字檔寫入本來就接受 `public/` 以外的 `.svg`（Code 模式介面目前不能新建，專案匯入會帶進來），所以這層不等 `public/` 開放。`public/` 的 SVG 仍拒絕，直到共用的 SVG 驗證器接上並在預覽與正式路徑以真實瀏覽器驗證。驗證器是 `src/lib/security/svg-validation.ts` 的 `validateSvg`：嚴格 XML 解析（`saxes`）＋元素允許清單＋以 css-tree 解析 CSS，不合格即拒絕並說明原因，**不清理、不改寫**，通過的檔案逐位元組保存；結果帶 `SVG_VALIDATOR_VERSION`，規則一改就遞增，舊版本的通過紀錄不得當作新規則的通過。媒體庫與 Theme `public/`（及日後專案匯入）必須共用它，各自只保留大小限制與錯誤訊息；不得另寫字串比對的 SVG 檢查。媒體庫上傳記錄 `svgValidatorVersion`（`src/lib/asset/svg-delivery.ts`），只有記錄等於目前版本的檔案可在允許的路由 inline，舊的 `svgValidated` 標記、較舊版本或未記錄者一律以 attachment 送出，直到重新驗證；每個媒體庫 SVG 回應不論標記都帶隔離標頭。任何資格（inline、複製進 `public/`、發布）都不得只憑儲存的標記給予，複製時重讀位元組驗證；新增送出路徑時必須一併套用並加入 `scripts/verify-svg-isolation.ts`（`pnpm verify:svg-isolation`，需三種瀏覽器與 `wrangler dev`，不在 CI；改動任何送出路徑時手動執行）。
...
- **搬移與複製走同一個批次**：二進位檔以 `binaryCopies`（`from`、`to`、來源 id／version）放到新路徑，搬移再把來源列入同批 `deletions`；文字檔與二進位檔同一交易完成，不得只處理一半。digest、MIME、大小只由伺服器從來源列讀取，**不得採信 client 傳來的 digest**——blob 以內容定址、跨 Theme 共用，採信它等於允許引用任何已存檔案。目的地必須原本不存在（本批要刪的也不行，互換與串接一律拒絕）、副檔名格式不變、留在 `public/`，額度以整批套用後的結果判定（複製時來源仍計入）。
- **改變網址前先審查**：搬移、重新命名與刪除會讓 `/…` 網址失效，必須在寫入**之前**顯示網址變化、Theme 原始碼中完整寫出的引用位置，以及「執行時組出的網址與頁面內容未檢查」；不得顯示「沒有引用」。預設提供「複製並保留舊網址」。
- **引用只在確認後、由伺服器改寫**：搬移或複製可帶 `publicUrlRewrite`，讓 Theme 原始碼中的引用跟著新網址走。規則：
  - 只改能以語法證明是完整網址的位置（`planPublicUrlRewrites`）：TS/JS 以 Babel 找字串、JSX 屬性、沒有 `${}` 的 template，值即網址（可帶 `?`、`#`）或為 `url(…)`（含 Tailwind `bg-[url(…)]`）；CSS 以 `css-tree/parser` 找 `url()` 與字串。**不得用 regex 全文取代**。
  - 其他寫出網址的地方一律列為「未更新」並附原因：執行時組出、跳脫字元、較長文字中（如 `srcSet`）、新網址在該處需跳脫、語法錯誤、非 TS/JS/CSS 檔。全文比對作最後一道網：凡寫出網址而未被改寫、未被列出、也不在註解內者，都要列出。Document 內容與外部網站不檢查，介面必須說明，不得宣稱已全部修正。
  - **伺服器自行重算**：以已存內容疊上本批寫入重新規劃，只接受本批 `binaryCopies` 中的來源→目的地，結果（檔案、改寫數、未更新數）與作者確認的不同就整批拒絕（`PUBLIC_URL_REWRITE_STALE`）；改寫內容不得取自 client。被改寫的檔案帶各自版本，與圖片搬移同一交易。
  - 會刪掉舊檔且仍有未更新引用時，需作者明確確認（`acknowledgeUnresolved`），伺服器也檢查；此時預設動作是「複製並更新」，保留舊網址。
  - 有未存檔草稿提到這些網址（且不在本批寫入中）時不提供自動改寫，請作者先存檔或捨棄。
  - 改寫進入工作區草稿，發布後正式網站才會改變；歷史 release 不受影響。
