...
- **SVG 先隔離送出，才可能接受上傳**：平台送出的每個 `image/svg+xml` 回應都帶 `theme-svg-isolation.ts` 的 `SVG_ISOLATION_HEADERS`（`sandbox` CSP、`default-src 'none'`、`nosniff`），`<img>` 照常顯示，直接開啟或當文件嵌入時不得執行腳本。四條送出路徑都要有：Live Preview 的 Vite 中介層（產生的 `vite.config.ts`，Sandbox 與本地 sidecar 共用）與 Sandbox 預覽代理、`serveThemeArtifact`（建置預覽與正式店面的 client 檔案）、正式店面 Theme Worker 的回應、Theme Worker 靜態資產（部署器寫入的平台 `_headers`；建置產物自己的 `_headers`／`_redirects` 一律不部署）。伺服器的文字檔寫入本來就接受 `public/` 以外的 `.svg`（Code 模式介面目前不能新建，專案匯入會帶進來），所以這層不等 `public/` 開放。`public/` 的 SVG 仍拒絕，直到共用的 SVG 驗證器（解析＋允許清單，不合格即拒絕、不清理改寫）完成並在預覽與正式路徑以真實瀏覽器驗證；新增送出路徑時必須一併套用並加入 `scripts/verify-svg-isolation.ts`（`pnpm verify:svg-isolation`，需三種瀏覽器與 `wrangler dev`，不在 CI；改動任何送出路徑時手動執行）。
