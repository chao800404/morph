...
CREATE TABLE `__new_storefront_releases` (
	...
	FOREIGN KEY (`content_publication_id`) REFERENCES `storefront_content_publications`(`id`) ON UPDATE no action ON DELETE restrict
);
...
CREATE INDEX `storefront_releases_theme_build_idx` ON `storefront_releases` (`theme_build_id`,`deleted_at`);