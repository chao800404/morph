...
> Release、OCC、本機 fail-closed 程式鏈，以及隔離 iframe 中的真實 TSX runtime 不回退；但真實
> Cloudflare Theme Worker、custom domain、remote migration、Publish E2E 與 Page Registry 尚未完成，
...
- 階段 7（12）：editor Live Preview 已是容器內的真實 Vite dev server,hooks、Fast Refresh 與
...
- **搬移時更新網址引用（3a）**：搬移或複製 `public/` 檔案時，Theme 原始碼中能以語法證明是完整網址的引用（TS/JS 用 Babel、CSS 用 css-tree），
  經作者確認後由伺服器重新規劃、與搬移同一交易改寫；執行時組出、跳脫、`srcSet` 等一律列為「未更新」並附原因，全文比對作最後一道網。
...
Theme 的路由從檔案集合推導，所以一個頁面就是一個原始碼檔案，沒有任何註冊表要維護。

- 位址在兩處各規劃一次：對話框每次按鍵都算一遍，讓走不通的路徑在還在打字時就被拒絕；伺服器再算一次，並把整張路由表連同新檔案重建，因為讓 Theme 無法描述自己路由的頁面必須在還能便宜拒絕時拒絕。
- 兩者都以**編輯器持有的** generation 做 OCC。伺服器自行讀取當下值會讓並行修改被無聲吞下。
- 建立與刪除都觸發預覽重新規劃，而不是增量同步：`__entry.tsx` 列出建置當時的路由，只有重新規劃才會重生。
- 刪除先寫 revision。這是編輯器唯一會直接銷毀成果的動作，rollback 是唯一的回頭路。

### 暖容器的 workspace 協調（2026-09-13）

材料化時會移除舊計畫留下的一般檔案 —— 被刪掉的元件仍能滿足舊的 import，預覽會繼續渲染編輯器已經不擁有的來源。只動 `type === "file"`，symlink 與目錄不碰（`/workspace/node_modules` 指向 image 裡釘死的工具鏈），fingerprint marker 是平台中繼資料也排除在外。列檔失敗直接丟錯：寫入新計畫到一個沒人描述得出的 workspace，之後提交的 fingerprint 會替一場沒發生的協調背書。

### Live Preview 改為真實 React 執行期（2026-09-13）

編輯器框住的是容器內一台真實的 Vite dev server，執行 Theme 自己的 JavaScript。作者仍然不需要手寫任何 `data-*`：定位與欄位標記由預覽編譯器在 AST 注入，正式 build 以 `strip-editor-markers` 移除。

- `useState`／`useEffect`／事件處理與合法 npm 套件都真實執行；Fast Refresh 熱更新 166ms 且保留狀態（對比整頁重載 279ms）。
- 重複 row 的身分在真實 React 下穩定（`items.0/1/2` 對應固定 item id）,section 身分以 `display:contents` 包裝維持。
- 相容性直譯器（`safe-theme-component-renderer.tsx` 1887 行、預覽路由 2243 行）仍在原始碼中，但編輯器已無法抵達 —— `resolveLivePreviewSource` 只能回傳 `preview-server` 或 `null`。觀察期結束後刪除。

### 預覽生命週期與 sandbox 存活（2026-09-13）

`live-preview-lifecycle.ts` 是唯一的擁有者；所有帶 key 的事件都會拒絕已被替換的 frame。

- 自動重連一次，再失敗才顯示 Retry；額度依上一段健康期長度決定是否補回。
- 每分鐘向 sandbox 查詢一次，該呼叫同時續期 idle 期限。編輯器關閉即停止查詢，容器照原本的 10 分鐘自己休眠 —— 不需要清理路徑，也不會為離開的作者釘住容器。
- 只有明確的「不在服務中」才觸發重連；請求本身失敗（掉包、逾時）不動作。

### Header／Footer 收斂為 Document slot（2026-09-10）

- Header／Footer 原本寫死在 layout 的 TSX 裡，內容從未進資料庫，右側 Content Fields 因此
  永遠是空的。改為與 section 完全相同的一條路：layout 宣告
  `content("starter-header")`／`content("starter-footer")` 兩個 slot，新增 `layout`
  template type 與預設 layout document，componentRef 使用 `header.default`／`footer.default`。
- 資料、原始碼、推導、編輯器、runtime、發布、renderer 七層一次打通，不是只補 UI。
- Inspector 對 hero／header／footer 三種 section 解析欄位的路徑已驗證為同一條
  （componentRef → capabilities → 元件自身的 `contentFields`），連結控制項三者完全相同。

### 連結欄位統一與 `ThemeLink`（2026-09-10）

- 所有連結欄位統一為 `type: "link"`：Hero／ImageWithText 的 `actionHref`、CategoryShowcase
  每列的 `href` 全部收斂，不再有一個欄位一種寫法。
- 新增共用的 `ThemeLink`：由值本身決定要渲染 `<a>` 還是 router 的 `<Link>`，
  外部網址不走 router、站內連結不整頁重載，這個判斷只寫一次。
- 新增 `ThemeContentFields` 型別，讓 `contentFields` 在編輯器裡就能得到型別檢查，
  而不是等平台讀取工作區時才發現拼錯。
- 開發資料庫的既有內容已一併遷移（`actionHref` → `action`、列的 `href` → `link`、
  `navItems` → `items`、`layout.*` → `*.default`），舊鍵直接移除而非留下孤兒資料。

### 解釋器與真實 React 的兩處分歧（2026-09-10）

- `typeof` 原本回傳 `undefined`：`typeof v === "string"` 對字串為假，
  ThemeLink 的字串分支從不執行，預覽裡的連結靜靜地少了 href，而真實建置是對的。
  未實作的運算子現在明確拒絕，而不是用 `undefined` 回答。
- JSX `key` 在元件元素上被丟棄（清單無 key），同時又被當成一般 prop 往下傳、
  經 spread 流進 DOM。現在 key 會接回節點，且不再當 prop 傳遞。

### Inspector 欄位判定（2026-09-10）

- 寫死綁 `actionHref` 的 Action Button 卡片會攔截宣告的 `action`，讓作者在一個
  沒有任何程式碼會讀的 prop 上打字。該卡片的目的地半邊現在只在元件真的有
  `actionHref` 時出現。
- 選取共用元件（`link.tsx`）畫出來的元素時，`contentFields` 會去共用檔案找而整組失效
  —— 只要用了共用元件就會踩到。改為找不到時再問 section 自身的元件。
- 欄位配對不再比對 `action`／`actionLabel` 這組名字，而是問 AST
  「跟選取欄位畫在同一個 JSX 元素上的是哪個 link 欄位」，
  因此 `ctaLabel`／`ctaLink` 這類命名同樣成立；選取外層容器時也會依後代欄位解析。
- 宣告欄位與後代綁定原本會把同一個欄位畫兩次，作者看到兩個一樣的輸入框卻分不出
  哪個算數。判斷規則抽成一份，兩個清單共用。
...
- [ ] 讓 Page Registry 組合 Theme build route manifest 與有權限的 D1 Page records，並支援 route navigation、空白頁建立與 Design 編輯。（**空白頁建立與刪除已完成**：Pages 面板的 Add／Delete page，路由由檔案集合推導，無產生檔要回寫；Page Registry 本身仍未組合 D1 Page records）
- [x] 在隔離 iframe 中執行目前 theme 的真實 TSX/component tree。（容器內 Vite dev server，獨立 hostname）
- [ ] 定義安全的模組載入、允許清單與 runtime 邊界。（容器內已有 dependency enforcer 與 workspace containment；公開 runtime 邊界仍未定案）
- [x] 讓 Inspector draft 覆寫套用到真實元件，而非固定 Storefront 相容 renderer。
- [x] Runtime 編譯或渲染失敗時保留上一個可用畫面，並提供明確錯誤狀態。（單一生命週期狀態機：啟動中／載入／同步／重連／失敗）
...
### 已解決：被環境錯誤遮蔽的 4 個測試失敗（2026-09-10）

`better-sqlite3` 的原生 binding 未編譯，16 個測試檔一律以 `Could not locate the bindings file`
失敗。這些檔案長期被當成「環境問題」略過，實際上把 4 個真實失敗一起蓋住了。重建 binding
後才浮現，四項均已修復。

**1. componentRef 嚴格拒絕 vs 內容保存 —— 已裁決：兩者都保留**

原本看似衝突的兩個設計，其實在講不同的事，是那個測試把它們併成一句斷言：

- **拒絕**針對的是**傳入的值**：Theme 認不得的 ref 無從對照任何宣告，客戶端送來的東西不可信。
- **保存**針對的是**已存的值**：它在寫入當下就驗證過了，不是客戶端此刻的主張，不該重驗、更不該清空。

裁決結果：

- `storefront-theme.dal.ts` 移除「認不得的 ref 回退到 section type 預設」。回退是在治標
  —— 真正造成抹除的是把**既有** props 也送進同一個過濾器，那個已由 `cleanExistingProps` 堵住。
  回退還會把內容寫到文件從未指名的元件上，並掩蓋真實的 ref 不一致。
- 但保留「**完全沒有** ref」時以 section type 補齊：那是舊文件的正常狀態，與「指名了一個不存在的元件」不同。
- 測試改名為 `refuses incoming props for an unrecognized componentRef without erasing stored ones`，
  斷言拆成三項：既有 `heading` 保留、未宣告的 `customProp` 被擋、`componentRef` 維持文件原本指名的值。

**2. LocalDirectThemeRuntime 的 3 個失敗 —— 測試環境問題**

`RequestInit: Expected signal to be an instance of AbortSignal`。這些是 runtime transport，
會用 `new Request(target, request)` 從一個請求造出另一個，而該建構會複製來源的 `AbortSignal`。
在 jsdom 下 `AbortSignal` 來自 jsdom、`Request` 仍來自 undici，undici 不認得那個 signal 就丟錯
—— 這個錯誤只存在於測試環境，Workers 只有一個 realm 也沒有 DOM。該檔案標上
`// @vitest-environment node`，並在檔頭寫明原因。

**尚未驗證：E2E。** `pnpm test:e2e` 本輪 25 項全部 skip，因為缺
`E2E_EMAIL`／`E2E_PASSWORD`／`E2E_EDITOR_PATH`。這不是失敗，但也不算通過；
第 8 階段的折減保留到實際跑過為止。
...
- [x] 先完成一個真實 section 的 TSX runtime 渲染。（整個 theme，非單一 section）
- [x] Inspector 修改可以即時套用，但不寫入 source/public content。
- [x] 編譯失敗能回復上一個成功版本。
- [x] 完成 focused tests 與安全邊界 review。
- [ ] 觀察期結束後刪除已無法抵達的直譯器路徑。
...
### 2026-09-17 第二十六輪：編輯器狀態 seam 與 workspace parity 修正

- 把 VisualEditorShell 的 template／route context reset 收斂為共用 hook，保留原本對 `undefined → id` 的非對稱語意，並用純函式測試鎖住切換邊界。
- 把 Inspector 的 content props rebase 邏輯抽成可測試 hook，切換 section 時重建快照，同一 section 只重算未被本地修改的欄位。
...
| 日期       | 階段／內容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 驗證                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-21～22 | **section 資料夾慣例、未綁定候選的一鍵綁定，與受限的 props 推導（第二十八輪）**：Theme 原本只能靠 manifest 註冊或匯出 `contentFields` 來說「這是 section」，兩者都會被忘記；忘記時元件照常 render，只是永遠不出現在 Add section，而且沒有任何訊息說明原因。`src/components/sections/` 成為第三條路：放進去就是候選，不必註冊也不必宣告（入口只有 `Hero.tsx` 與 `hero/index.tsx` 兩種形狀，測試檔、資料夾根的 `index`、再深一層都不算）。路由直接 render 而沒有 `content(...)` 的位置會列在「Sections needing binding」並可一鍵綁定；只有直接 JSX 位置可綁，條件式／`map` callback／任意 expression 一律附原因拒絕，因為同一個 source 位置可能代表 0 個或多個渲染實例，一個 slot 描述不了。綁定前以當前 source 重新推導並比對位置、元件名與來源檔案，source 已變更時拒絕寫入而不是補到別的元件上。`{...content("...")}` 插在元素自身屬性**之前**，因此作者寫的顯式 prop 優先於 slot 值。沒有 `contentFields` 時改從 source 推導欄位（字面量預設值、行內型別標註、同檔 `type`／`interface`）；需要型別檢查器才讀得到的與執行期值一律不推導，`v?: string` 與 `v: string \| undefined` 視為同一件事。已宣告的 `contentFields` 一律優先，宣告無效時也不退回推導。候選會標示綁定寫進哪個檔案：`This page` 寫進 route，`Every page` 寫進 layout 並影響所有頁面。Add section 與 Bind 共用同一組 `content(...)` 寫入與 import 解析。 | `pnpm typecheck` 0 錯誤；聚焦 122 passed；`pnpm test` **2762 passed、7 failed、1 skipped**（357 檔 —— 7 個失敗為 Windows 路徑環境問題，非本次改動，見上方「最近完整驗證」）；`pnpm build` 含 client bundle check、local preview sidecar、deploy artifact guard 全數通過；`git diff --check` 乾淨；改動檔 Prettier 乾淨。關鍵守衛與推導皆做過變異測試（中和守衛／還原修正前的 predicate 會精確紅掉對應測試）。仍未執行 `pnpm test:e2e`。 |
| 2026-09-14 | **Code／Design 草稿同步與樣式清除一致化（第二十三輪）**：Code 編輯器的草稿會即時同步到真 React Live Preview，閒置約 700ms 自動儲存，`⌘S`／`Ctrl+S` 仍可立即儲存；切換 Design／Code 不再重載或跳出未儲存對話框。同步失敗與 OCC 衝突會保留草稿並提示。Responsive 背景與邊框清除也統一為可逆的透明 reset，並同步畫布預覽。 | `pnpm typecheck` 0 錯誤；`pnpm test` **2367 passed、1 skipped**；`pnpm build`、client bundle check、deploy artifact guard、`git diff --check` 通過；本輪未執行 `pnpm test:e2e`（缺 `.env.e2e` 憑證）。 |
| 2026-09-13 | **頁面生命週期，與兩次「修法比原問題更糟」（第二十二輪）**：Pages 面板補上 Add page 與 Delete page。路由由檔案集合推導，容器裡根本沒有 `routeTree.gen.ts`，所以新增一頁就是寫一個檔案、沒有產生檔要回寫；但預覽的 `__entry.tsx` 列出建置當時的路由，只在 workspace 重新規劃時重生，因此建立與刪除都走重試路徑而非增量同步。範本用 `<main></main>` 而非 `<main />`，因為加 section 是插在閉合標籤前，自閉合會做出一個永遠加不了東西的頁面。刪除的守衛比新增嚴：`__root` 是每頁渲染所在的殼、首頁刪掉會讓編輯器刪除當前頁後的安全去處自己消失、最後一頁刪掉就無法發布（計數要跳過 root，否則空主題看起來仍有一條路由），並先寫 revision。**兩個我自己造出來的缺陷**：（一）新增頁面原本由伺服器自行讀取當下 generation，於是別人的並行修改會被一起吞下，編輯器接著把那個被污染的值標記為已接受，下一次存檔就會覆蓋掉沒看過的變更 —— 前一個 commit 的訊息還宣稱相反。改為由編輯器帶入持有的 generation、伺服器直接以它做 OCC。（二）預覽存活檢查改用 `getExposedPorts` 之後，實測發現它對一個 curl 回 200 的預覽回空陣列，於是健康的預覽每 60 秒被拆掉重建一次；再者本機存取會改寫協定與 port，比對 origin 永遠不相等。收斂為比對 hostname，且只有「清單列了別的位址、就是沒有這一台」才判定失效，空清單不下結論。另修復滾輪失效：預覽鋪滿畫布，捲動與 Ctrl 縮放都得由預覽轉發，而轉發只在相容性直譯器裡實作過，編輯器不再載入它之後就安靜消失 —— **沒有任何測試失敗**，因為訊息型別、parser 與編輯器處理器都還在，缺的只是發出端。監聽器明確設為非被動，否則 passive 監聽器無法 `preventDefault`，瀏覽器會在畫布縮放的同時縮放預覽文件本身。workspace 也補上協調：暖容器會移除舊計畫留下的一般檔案，但不動 symlink 與 `node_modules`，列檔失敗則直接丟錯而非默默跳過。 | `pnpm typecheck` 0 錯誤；`pnpm test` **2338 passed、1 skipped**；`pnpm build` 含 client bundle check（339 檔）與 deploy artifact guard 通過；`git diff --check` 乾淨。實機驗證：新增頁面後 `__entry.tsx` 由四條路由重生為五條並可加入 section；刪除三個測試頁後回到四條，且暖容器上一併移除孤兒檔案而 `node_modules` symlink 與 fingerprint marker 完好；故意送出過期 generation 會被拒絕且不建立頁面；健康預覽連續 124 秒零次誤判重連（修正前每 60 秒一次）；滾輪捲動實測父視窗收到 0 個 wheel 事件、bridge 轉發 `dy=300`、畫布位移 105 → −195。每個回歸測試都以「撤回修正後必須失敗」反向驗證過。**Ctrl+滾輪的縮放無法用工具送出修飾鍵，由使用者回報確認**；**本輪未執行 `pnpm test:e2e`**（spec 已寫，缺 `.env.e2e` 憑證）。 |
| 2026-09-13 | **Live Preview 收斂為單一真實 React 引擎（第二十一輪）**：相容性直譯器退出編輯器路徑 —— `LivePreviewSource` 收斂為單成員型別、`executionMode` 寫死 `user-code`，不再有「缺 hostname／容器啟動中／啟動失敗就默默換一套語意」的分支。預覽生命週期收斂為單一狀態機（啟動中→載入→同步→完成→重連→失敗），取代先前三個條件互相重疊又互不覆蓋的計時器。過程中修掉四個只有實機才看得到的缺陷：（一）**編輯器每次同步都覆寫容器自己產生的 `package.json`** —— 兩者永遠不同，`changed` 因此永遠非空，而 `package.json` 不在任何 module graph,HMR 確認永遠不會到，15 秒後謊報「未確認」。（二）**frame 從未載入時沒有任何計時器武裝** —— 確認逾時要等 frame ready，心跳要等來源確認，兩者都到不了，畫布無限轉圈超過五分鐘。（三）**失敗狀態被重複的 `server-ready` 復活** —— query 握著上一次成功結果，每次 render 都重送，把剛放棄的 frame 推回載入中並清掉訊息，Retry 永遠留不住（實測卡 96 秒）。（四）**心跳只證明文件活著，不證明 sandbox 活著** —— 容器死後瀏覽器裡那份頁面照常回 pong，編輯器對著 410 的預覽顯示 ready。改為每分鐘問 sandbox 一次，該呼叫同時續期 idle 期限；刻意不用 `keepAlive: true`（官方語意是永不逾時且必須自行 destroy，分頁崩潰即洩漏）。恢復額度改為依「上一段健康期長度」判定：超過一分鐘視為新插曲並補回額度，短於一分鐘視為 flapping 不補。另完成 workspace 寫入最佳化：先驗證再接觸容器、目錄去重、最多 8 個並行，以及以最終位元組計算的 fingerprint 快速路徑（相同即完全跳過寫入）。                                                                                                                                                                                                                                                       | `pnpm typecheck` 0 錯誤；`pnpm test` **2308 passed、1 skipped**;`pnpm build` 含 client bundle check（339 檔）與 deploy artifact guard 通過；`git diff --check` 乾淨。實機量測：冷啟動 serverFn 7.1→**5.6 秒**（workspace 3.35→**1.93 秒**、mkdir 28→**8** 次）、暖啟動 **293 毫秒且零檔案寫入**（容器內三個檔案時間戳完全未動）、失聯偵測 12–15 秒後 **1.5 秒**恢復、續期下容器連續存活 **21 分鐘**（修正前 10 分鐘即休眠）、`docker stop` 後 **60.3 秒**偵測並於 68.3 秒恢復。每個回歸測試都以「撤回修正後必須失敗」反向驗證過。**本輪未執行 `pnpm test:e2e`。**                                                                                                                                                               |
| 2026-09-11 | 瀏覽器測試首次實際跑通，並修掉它揭露的三個真實缺陷（第二十輪）：（一）**首頁 section 拖曳排序對每個新店都是壞的**。顯示／隱藏被寫成包住 section 的判斷 `{!isSectionHidden("id") && <Hero />}`，starter 本身就產生這個形狀，於是沒有一個 section 是 `<main>` 的直接子節點，重排以「必須是直接兄弟」拒絕。改為搬移「只為容納該 section 而存在的最外層包裝」，包裝跟著各自的 section 走。（二）**Header／Footer 被放進可排序清單**——它們自從成為 slot 後會併進頁面的 section 清單，拖任何一列都會把路由未宣告的 slot id 送進重排，寫入被拒、只顯示「Save failed」。這是 slot 化的回歸；現在排除在排序之外、改以「All pages」外框列呈現，footer 也回到正確的最後位置。（三）樣式面板可編輯 `cn("靜態", 條件 && "x")` 的靜態半邊，條件原封不動；順帶修掉 `cn("a","b")` 寫回時重複 class 的既有 bug。另有三項失敗是測試本身過期（用原始碼位置當唯一識別、用列文字判斷選取、拖到不可拖的列），以及一個 `test.fail` 的已知回歸因 Inspector 停止每次 render 重新解析原始碼而恢復，已改回一般閘門。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `pnpm typecheck` 0 錯誤；`pnpm test` **300 files / 2110 tests：299 passed、1 skipped、0 failed**；`pnpm build` 含 client bundle check（339 檔）與 deploy artifact guard 通過；**`pnpm test:e2e` 24 passed / 0 failed / 2 skipped**（兩項 skip 為刻意閘門）。另在編輯器中對真實工作區逐項確認樹狀結構與拖曳行為。                                                                                                                                                                                                                                                                                                                                                                                                                |
| 2026-09-10 | 修復被環境錯誤遮蔽的 4 個真實失敗，並裁決 componentRef 邊界（第十九輪）：`better-sqlite3` 原生 binding 未編譯，16 個測試檔長期以環境錯誤失敗，把 4 個真實失敗一起蓋住。重建 binding 後：（一）`componentRef` 嚴格拒絕與內容保存被判定為**不衝突** —— 拒絕針對傳入值、保存針對已存值，是原測試把兩者併成一句斷言。移除「認不得的 ref 回退到 section type」（治標，且會把內容寫到文件未指名的元件上），保留「完全沒有 ref 時以 section type 補齊」，測試改名並拆成三項斷言。（二）`LocalDirectThemeRuntime` 三項為 jsdom 的 `AbortSignal` 跨 realm 問題，該檔案改用 node 環境並在檔頭寫明原因。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `pnpm typecheck` 0 錯誤；`pnpm test` **292 files / 2063 tests：291 passed、1 skipped、0 failed**（本輪首次全綠）；`pnpm build` 含 client bundle check（332 檔）與 deploy artifact guard 通過；`prettier --check` 乾淨。`pnpm exec playwright install chromium` 後重跑 `pnpm test:e2e`，**25 項全部 skip** —— 缺 `E2E_EMAIL`／`E2E_PASSWORD`／`E2E_EDITOR_PATH`，憑證應由使用者提供，不代填。                                                                                                                                                                                                                                                                                                                                    |
| 2026-09-10 | Header／Footer 收斂為 Document slot、連結欄位全站統一，並修復 Inspector 的欄位判定（第十八輪）：Header／Footer 原本寫死在 layout 的 TSX 裡，內容從未進資料庫，右側 Content Fields 因此永遠是空的；改為與 section 同一條路 —— `content("starter-header")`／`content("starter-footer")` 兩個 slot、`layout` template、`header.default`／`footer.default` componentRef，資料／原始碼／推導／編輯器／runtime／發布／renderer 一次打通。連結欄位統一為 `type: "link"`（Hero／ImageWithText 的 `actionHref`、CategoryShowcase 列的 `href` 全部收斂），並以 `ThemeLink` 取代手寫 `<a>`／`<Link>` 分支，由值決定用哪個元素。解釋器補上 `typeof`（原本回傳 `undefined`，害 `typeof v === "string"` 對字串為假，連結靜靜地少了 href）與 JSX `key`（元件元素的 key 被丟棄，且當成一般 prop 流進 DOM）。Inspector 三個缺陷：寫死綁 `actionHref` 的 Action Button 卡片會蓋掉宣告的 `action`；選取共用元件（`link.tsx`）的元素時，`contentFields` 會去共用檔案找而整組失效；宣告欄位與後代綁定會把同一個欄位畫兩次。欄位配對改為問 AST「跟選取欄位畫在同一個 JSX 元素上的是哪個 link 欄位」，不再比對 `action`／`actionLabel` 這組名字。另修復 `better-sqlite3` 原生 binding 未編譯 —— 這件事讓 16 個測試檔長期以環境錯誤失敗，**遮蔽了 4 個真實失敗**（見下方「已知未解」）。                                                                                                                                                                                                                                                                                                                                                                                                     | `pnpm typecheck` 0 錯誤；`pnpm test` 292 files / 2063 tests（289 files passed、2 files failed；2058 passed、4 failed、1 skipped）；`pnpm build` 含 client bundle check（332 檔）與 deploy artifact guard 通過；`pnpm prettier --check` 乾淨。新增測試：`theme-link-rendered-field.test.ts`、`editor-action-button-declared-link.test.tsx`、`safe-theme-component-key.test.tsx`、`safe-theme-unary.test.tsx`、`theme-link-component.test.ts`。另以工作區真實的 `Hero.tsx`／`Header.tsx`／`Footer.tsx`／`link.tsx` 實跑直譯器與 Inspector 核對（非只靠合成樣本）。**本輪未執行 `pnpm test:e2e`。**                                                                                                                                |
