...
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 最後更新     | 2026-09-11                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 目前狀態     | 核心 Editor／Build／Release 已完成主要鏈路；Header／Footer 為 Document slot，連結欄位全站統一為 `type: "link"`。**瀏覽器測試首次實際跑通並全綠**，過程中揭露三個只有真實瀏覽器才看得到的缺陷（見更新紀錄）。Production Runtime、Domain 與遠端 Publish 仍未閉環，Live Preview 仍是直譯器而非真實 React                                                                                                                                                                                                                        |
| 整體完成度   | **92%**（E2E 由「全部 skip」變成 24 passed／0 failed，第 8 階段據此上調；第 7 階段維持，直譯器仍不支援 React hooks）                                                                                                                                                                                                                                                                                                                                                                                                         |
| 目前重點     | 真實 TSX Live Runtime（hooks 與合法 npm 套件在預覽中失效，是目前最大的架構斷層）；其次為商品 provisioning 觸發、真實 Cloudflare Theme Worker／Domain／Publish 閉環                                                                                                                                                                                                                                                                                                                                                           |
| 最近完整驗證 | 2026-09-11 實跑：`pnpm typecheck`（0 錯誤）、`pnpm test`（300 files / 2110 tests：**299 passed、1 skipped、0 failed**）、`pnpm build` 含 client bundle check（339 檔）與 deploy artifact guard、`prettier --check` 乾淨。**`pnpm test:e2e` 24 passed / 0 failed / 2 skipped** —— 兩項 skip 是刻意的閘門：`authored-content` 需 `E2E_SCRATCH_EDITOR_PATH`（專用拋棄式 theme），`publish` 需 `E2E_ALLOW_PUBLISH=1`（會建立 release 並把 production 切過去）。仍未執行：遠端 Publish、Cloudflare deploy、`pnpm db:migrate:prod` |
...
| 7. Live Runtime 與真實建置的一致性 | 解釋器輸出必須與真實 React、真實 router、catalog loader 與隔離的 TSX runtime 一致                                                                                                 | 🟡   |    80% | 完成真實 TSX/component iframe、模組邊界、公開 catalog runtime 與 Inspector 套用的完整回歸                                                                             |
