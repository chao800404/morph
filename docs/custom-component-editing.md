...
- 伺服器寫入內容時，以已儲存 source 中該 section 實際渲染的元件驗證欄位，不使用 Document 裡存的 `componentRef`：頁面 template 依編輯器目前顯示的 route（只能是 source 宣告過、且屬於該 template 的 route，否則拒絕寫入），layout 依 layout source。source 無法回答時（沒有 route、route 有診斷錯誤、沒有宣告該 slot）才沿用儲存的 ref。
