...
- Starter 的首頁同樣採用頁面副本：新 workspace 的 `src/routes/index.tsx` 直接 import `page-sections/index/<slot>/index.tsx`；既有 workspace 的首頁若仍是未修改的 Starter 版本，升級到 Starter 版本 27 時會一次改成副本，任何一個 section 無法複製就整個不動。改過的首頁不自動轉換，改由 Template 標示與 detach 處理。新 workspace 的範本庫直接放實作：`src/components/sections/Hero.tsx` 就是 Hero 本身，沒有 `src/components/Hero.tsx`。既有 workspace 維持原結構（`sections/Hero.tsx` 轉出 `src/components/Hero.tsx`），因為升級邏輯以該路徑的內容辨認 Starter 版本，升級也不搬動作者的檔案。
- 伺服器寫入內容時，以已儲存 source 中該 section 實際渲染的元件驗證欄位，不使用 Document 裡存的 `componentRef`：頁面 template 依編輯器目前顯示的 route（只能是 source 宣告過、且屬於該 template 的 route，否則拒絕寫入），layout 依 layout source。這是 fail-closed：只要 route（或 layout）透過 `content(...)` 決定結構，source 無法確認該 section 就拒絕寫入（`SECTION_SOURCE_UNCONFIRMED`，並回傳原因），包括 route 有診斷錯誤、沒有宣告該 slot、route 檔無法組成有效的 route tree、找不到這個 template 的 route、編輯器指定的 route 不存在或不屬於該 template。只有 source 沒有意見的舊 Theme（沒有任何 route 檔，或 route／layout 尚未採用 `content(...)`）才沿用 Document 儲存的 ref。
- 內容存在哪份 Document，由 route 路徑決定，編輯器、伺服器與正式 runtime 共用同一條規則（`contentTargetForRoutePath`）：`/` 用 index，`/products/…`、`/collections/…`、`/blogs/…`、`/pages/…` 用各自類型的共用 template；其他**靜態** route（如 `/aboutus`、`/contact`，以及列表頁 `/products` 本身）各自擁有一份以 `route_path` 綁定的 Document，第一次寫入內容時才建立，發布後 runtime 依精確路徑提供。多個靜態頁不共用一份 `page` Document，否則寫一頁會丟掉另一頁的 section。含參數、又不在上述前綴下的 route（如 `/journal/$slug`）目前沒有內容 Document，編輯器與伺服器會明確回報。
