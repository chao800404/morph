...
- Starter 的首頁同樣採用頁面副本：新 workspace 的 `src/routes/index.tsx` 直接 import `page-sections/index/<slot>/index.tsx`；既有 workspace 的首頁若仍是未修改的 Starter 版本，升級到 Starter 版本 27 時會一次改成副本，任何一個 section 無法複製就整個不動。改過的首頁不自動轉換，改由 Template 標示與 detach 處理。`src/components/Hero.tsx` 等實作仍留在原位（範本透過轉出指向它們）。
