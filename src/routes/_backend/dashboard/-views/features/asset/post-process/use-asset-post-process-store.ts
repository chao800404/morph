...
export type PostProcessItem = {
  ...
  extension?: string;
  size?: number;
  onUpdated?: (result: { src: string; size: number }) => void;
};
...
export const useAssetPostProcessStore = create<AssetPostProcessState>(
  (set) => ({
    open: false,
    activeItem: null,
    baseImageSrc: null,
    ...
    resetAdjustments: () =>
      set({ adjustments: defaultAdjustments, crop: null }),
    reset: () =>
      set({
        open: false,
        activeItem: null,
