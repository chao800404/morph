...
export const AssetPostProcessDialog = () => {
  ...
  const getProcessedImageFile = async (): Promise<Blob | null> => {
    ...
    try {
      ...
      // Check if any adjustments (brightness, contrast, saturation, hue, blur) are active
  ...
  const handleProcessImage = async (saveMode: "update" | "new") => {
    ...
    try {
      ...
      formData.append(
        "croppedImage",
        blob,
        `${filename}_cropped_${width}x${height}.png`,
      );
  ...
  return (
    <Sheet open={open} onOpenChange={handleChange}>
      <SheetContent
        ...
        className={cn(
          "bg-transparent border-l-0 p-2 shadow-none w-full",
          "sm:max-w-full",
        )}
      >
        {open && (
                  ...
                  >
                    <div className="mb-2">
                      ...
                      <p className="text-zinc-500 text-sm">
                        {
                          tabsConfig.find((t) => t.value === activeTab)
                            ?.description
                        }
                      </p>
                    </div>
                                      ...
                                      onClick={() =>
                                        cropperRef.current?.setCoordinates(
                                          (state: any) => {
                                            ...
                                            if (!coordinates)
                                              return coordinates;
                                            return ratio.transform(coordinates);
                                          },
                                        )
                                      }
                                    >
