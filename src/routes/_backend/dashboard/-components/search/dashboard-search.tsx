...
import { globalSearchQueries } from "@queries/global-search.queries";
...
export const DashboardSearch = () => {
  ...
  const navigationNamespaces = useMemo(
    () => [
      {
        basePath: "/dashboard",
        groups: getConfig().client.collections.global,
      },
      {
        basePath: "/dashboard/settings",
        groups: getConfig().client.collections.settings,
      },
    ],
    [],
  );
  ...
  const { data, isError, isFetching } = useQuery({
    ...globalSearchQueries.results({ query, area, limit }),
    enabled: open && canSearch,
  });
  const navigationGroups = useMemo(
    () =>
      navigationNamespaces.flatMap(({ basePath, groups }) =>
        groups.map((group) => ({
          title: group.title,
  ...
  const preloadHref = (href: string) => {
    ...
    const slug = segments[isSettings ? 2 : 1];
    if (!slug) return;
    ...
    const recordSegment = segments[isSettings ? 3 : 2];
    const pageSegment = segments[isSettings ? 4 : 3];
    const view =
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button type="button" variant="cardHeader" size="xs">
                {
                  GLOBAL_SEARCH_AREA_OPTIONS.find((item) => item.value === area)
                    ?.label
                }
                    ...
                    {group.hasMore && area === group.area ? (
                      <CommandItem
                        ...
                        onSelect={() =>
                          setLimit(
                            (current) =>
                              current + GLOBAL_SEARCH_LIMIT_INCREMENT,
                          )
                        }
                      >
                        <Plus className="size-4" />
