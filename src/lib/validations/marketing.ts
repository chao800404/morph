...
export const createOrderInputSchema = z.object({
  ...
  currencyCode: z.string().trim().length(3).transform((value) => value.toLowerCase()),
...
export const updateOrderInputSchema = z.object({
  id: z.uuid(),
  email: z.email().optional().or(z.literal("")),
  status: z.enum(["pending", "completed", "draft", "archived", "canceled", "requires_action"]),
  noNotification: z.coerce.boolean().default(false),
});
...
export const promotionInputSchema = z.object({
  ...
  currencyCode: z.string().trim().length(3).transform((value) => value.toLowerCase()).optional().or(z.literal("")),
  ...
  campaign: z.object({
    ...
    budgetType: z.enum(["spend", "usage", "use_by_attribute", "spend_by_attribute"]),
