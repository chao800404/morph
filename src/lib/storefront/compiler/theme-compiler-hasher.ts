...
export function serializeCompilerInput(
  ...
  const payload = {
    ...
    compilerVersion: compilerIdentity?.version ?? input.compilerVersion ?? "4.1.17",
...
export function sha256(ascii: string): string {
  ...
  for (i = 0; i < ascii.length; i++) {
    ...
    if (code < 128) {
      ...
      words[currentByte >> 2] |= (0xc0 | (code >> 6)) << (24 - (currentByte % 4) * 8);
      currentByte++;
      words[currentByte >> 2] |= (0x80 | (code & 0x3f)) << (24 - (currentByte % 4) * 8);
      currentByte++;
    } else {
      words[currentByte >> 2] |= (0xe0 | (code >> 12)) << (24 - (currentByte % 4) * 8);
      currentByte++;
      words[currentByte >> 2] |= (0x80 | ((code >> 6) & 0x3f)) << (24 - (currentByte % 4) * 8);
      currentByte++;
      words[currentByte >> 2] |= (0x80 | (code & 0x3f)) << (24 - (currentByte % 4) * 8);
      currentByte++;
    }
