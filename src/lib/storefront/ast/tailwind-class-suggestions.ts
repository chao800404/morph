...
const BASE_SUGGESTIONS: TailwindClassSuggestion[] = [
  ...
  ...entries("Colors", [
    ...
    "border-black",
    "border-white",
    ...["bg", "text", "border", "ring", "outline", "fill", "stroke"].flatMap(
      (prefix) =>
        COLORS.flatMap((color) =>
          SHADES.map((shade) => `${prefix}-${color}-${shade}`),
        ),
    ),
  ]),
...
function splitVariantPrefix(query: string): {
  prefix: string;
  utility: string;
} {
  let bracketDepth = 0;
  ...
  return lastColon < 0
    ...
    : {
        prefix: query.slice(0, lastColon + 1),
        utility: query.slice(lastColon + 1),
      };
}
