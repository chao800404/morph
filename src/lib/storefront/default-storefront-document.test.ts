...
describe("default storefront home document", () => {
  it("creates a schema-valid, independently mutable starter document", () => {
    ...
    expect(STOREFRONT_STARTER_TEMPLATE_VERSION).toBe(2);
  ...
  it("only upgrades an empty or untouched legacy starter document", () => {
    expect(
      isUpgradeableStarterHomeDocument({ version: 1, sections: [] }),
    ).toBe(true);
    expect(isUpgradeableStarterHomeDocument(createDefaultStorefrontHomeDocument())).toBe(
      false,
    );
  });
});
